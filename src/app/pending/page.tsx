    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-brand-800 px-6 text-center">
      <div className="max-w-md rounded-xl bg-white p-8 shadow-xl">
        ...
        <h1 className="text-xl font-bold text-brand-800">
          {rejected ? "Zugang nicht freigegeben" : "Zugang wird geprüft"}
        </h1>
        <p className="mt-3 text-sm text-slate-600">
          {rejected
            ? "Ihr Zugang wurde nicht freigegeben. Bitte kontaktieren Sie unsere Kanzlei für weitere Informationen."
            : `Vielen Dank für Ihre Registrierung bei ${FIRM_NAME}. Ihr Zugang wird derzeit von unserer Kanzlei geprüft und in Kürze freigeschaltet.`}
        </p>
        <p className="mt-4 text-xs text-slate-400">
          Angemeldet als {profile.email}
        </p>
        <div className="mt-6">
          <SignOutButton>
            <button className="rounded-md border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
              Abmelden
            </button>
          </SignOutButton>
        </div>
